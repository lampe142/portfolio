...
```{r include = TRUE, echo=FALSE, fig.align='center'}
dp$risk %>% subset((!is.na(dp$risk$SharePortfolio) & dp$risk$SharePortfolio>0.0001) | dp$risk$Name == 'Portfolio:'
                   ,select=names(dp$risk)[c(2:18,27,28,37,40,42,43)]) %>%
...
columnDefs = list(list(visible=FALSE, targets=c(1,10:23))),
    dom = 'Bfrtip',
...
```{r include = TRUE, echo=FALSE, fig.align='center'}
dp$risk %>% subset(is.na(dp$risk$SharePortfolio) | dp$risk$Name == 'Portfolio:') %>%
  mutate(std_log_returns = std_log_returns * 100) %>% 
  mutate(std_log_returns_1Y = std_log_returns_1Y * 100) %>%
  mutate(VolaN1_MSGARCH = VolaN1_MSGARCH * 100) %>%
  dplyr::rename(`ind VaR MSGARCH`=ind_VaR_MSGARCH)%>%
  ...
  select (select=-c('Value', 'SharePortfolio','component VaR','VaR Share','..41')) -> risk.Table2

#risk.Table2 <- risk.Table2[,c(2:4,7:40,44,45)]

risk.Table2 <- replace(risk.Table2, is.na(risk.Table2),0)
datatable(risk.Table2, extensions = 'Buttons', options = list(pageLength = 100,columnDefs = list(list(visible=FALSE, targets=c(1,2,10:38))),
    dom = 'Bfrtip',buttons = list('copy', 'csv', 'excel', 'pdf', 'print',
list(extend = 'colvis',visible=F, targets= 0, columns = c(3:dim(risk.Table2)[2]))))) %>%
  formatRound('Beta', digits = 2, interval = 3, mark = " ")%>%
  formatStyle('Beta', background = color_from_middle(risk.Table$Beta,'pink','pink')) %>%
  formatRound('Beta1Y', digits = 2, interval = 3, mark = " ")%>%
  formatStyle('Beta1Y', background = color_from_middle(risk.Table$Beta1Y,'pink','pink')) %>%
  ...
  formatRound(names(risk.Table2)[17], digits = 3, interval = 3, mark = " ")%>%
  formatRound('incremental VaR', digits = 4, interval = 3, mark = " ")%>%
  formatRound("incremental VaR to Port VaR (share Portfolio VaR)", digits = 4, interval = 3, mark = " ")%>%
  formatRound('incremental VaR per Value', digits = 4, interval = 3, mark = " ")%>%
  formatRound('share Portfolio VaR / share portfolio', digits = 4, interval = 3, mark = " ")%>%
  ...
  formatPercentage('ES MSGARCH', 1) %>%
  formatStyle('ES MSGARCH', background = color_from_middle(risk.Table$`ES MSGARCH`,'pink','pink')) %>%
  formatPercentage(names(risk.Table2)[35], 1) %>%
  formatPercentage(names(risk.Table2)[36], 1) %>%
  formatPercentage('ES bootstrap', 1) %>%
  formatStyle('ES bootstrap', background = color_from_middle(risk.Table$`ES bootstrap`,'pink','pink')) %>%
  formatPercentage(names(risk.Table2)[18], 1) %>%
  formatPercentage(names(risk.Table2)[22], 1) %>%
  formatPercentage(names(risk.Table2)[23], 1) %>%
  formatPercentage(names(risk.Table2)[24], 1) %>%
  formatPercentage(names(risk.Table2)[26], 1) %>%
  formatPercentage(names(risk.Table2)[27], 1) %>%
  formatPercentage(names(risk.Table2)[28], 1) %>%
  formatPercentage(names(risk.Table2)[29], 1) %>%
  formatPercentage(names(risk.Table2)[30], 1) %>%
  formatPercentage(names(risk.Table2)[31], 1) %>%
  formatPercentage(names(risk.Table2)[32], 1) %>%
  formatPercentage(names(risk.Table2)[33], 1) %>%
  formatPercentage(names(risk.Table2)[34], 1) %>%
  formatPercentage(names(risk.Table2)[37], 1) %>%
  formatPercentage('VaR_bootstrap', 1) %>%
  formatStyle('VaR_bootstrap', background = color_from_middle(risk.Table$`VaR_bootstrap`,'pink','pink')) %>%
  formatPercentage('ind VaR MSGARCH', 1) %>%
  formatStyle('ind VaR MSGARCH', background = color_from_middle(risk.Table$`ind VaR MSGARCH`,'pink','pink'))

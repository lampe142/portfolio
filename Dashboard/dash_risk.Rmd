...
ts_a2 <-  xts::xts(x = cbind(-100 *portRisk$`VaR bootstrap 95Quantile`,
                            ...
                            100 * portRisk$MSGARCH_prob), 
                  order.by = as.Date(portRisk$CloseDate))
names(ts_a2) <-  c("lwr", "fit", "upr","MSGARCH prob")


dygraphs::dygraph(ts_a2, main = "Portfolio VaR", width = 1800) %>%
  dyAxis("x", drawGrid = FALSE) %>%
  dyAxis("y", label='VaR in %', drawGrid = FALSE) %>%
  dySeries(c("lwr", "fit", "upr"), label = "VaR 0.95") %>%
  dyOptions(colors = RColorBrewer::brewer.pal(3, "Set1")) %>%
  dySeries("MSGARCH prob", axis = 'y2') %>%
  dyAxis("y2", label="MSGARCH prob", drawGrid = FALSE) %>%
...
ts_a <-  xts::xts(x = cbind(-100 *portRisk$`ES bootstrap 95Quantile`,
                      ...
                      100 * portRisk$MSGARCH_prob), 
                 order.by = as.Date(portRisk$CloseDate))
names(ts_a) <-  c("lwr", "fit", "upr","MSGARCH prob")


dygraphs::dygraph(ts_a, main = "Portfolio ES", width = 1800) %>%
  dyAxis("x", drawGrid = FALSE) %>%

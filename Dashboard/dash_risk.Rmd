...
<p style="text-align: center;">Vola Index:</p>

<center>
```{r include = TRUE, echo=FALSE, fig.align='center'}
dygraphs::dygraph(xts(order.by = as.Date(portRisk$CloseDate), 
  x = portRisk[,c("VolaN1_MSGARCH","std_log_returns","std_log_returns_1Y")]),
  main = '', width = 1800) %>%
  dyRangeSelector(height = 20)
```
</center>

<p style="text-align: center;">VaR Index:</p>

<center>
```{r include = TRUE, echo=FALSE, fig.align='center'}
dygraphs::dygraph(xts(order.by = as.Date(portRisk$CloseDate), 
  x = portRisk[,c("VaR Normal",
                  "VaR Normal std1Y","VaR HS 1Y",
                  "ind_VaR_MSGARCH",
                  "VaR_bootstrap")]),  main = '', width = 1800) %>%
  dyRangeSelector(height = 20)
```
</center>

<p style="text-align: center;">ES Index:</p>
<center>
```{r include = TRUE, echo=FALSE, fig.align='center'}
dygraphs::dygraph(xts(order.by = as.Date(portRisk$CloseDate), 
  x = portRisk[,c("ES_MSGARCH",
                  "ES bootstrap 05Quantile",
                  "ES bootstrap 95Quantile",
                  "ES bootstrap")]),  main = '', width = 1800) %>%
  dyRangeSelector(height = 20)
```
```{r include = TRUE, echo=FALSE, fig.align='center'}
...
names(ts_a) <-  c("lwr", "fit", "upr","vola MSGARCH")


dygraphs::dygraph(ts_a, main = "Portfolio ES") %>%
  dyAxis("x", drawGrid = FALSE) %>%
  dyAxis("y", label='ES in %', drawGrid = FALSE) %>%
  dySeries(c("lwr", "fit", "upr"), label = "ES 0.95") %>%
  dyOptions(colors = RColorBrewer::brewer.pal(3, "Set1")) %>%
  dySeries("vola MSGARCH", axis = 'y2') %>%
  dyAxis("y2", label="vola MSGARCH", drawGrid = FALSE) %>%
  dyRangeSelector(height = 20)
```
</center>

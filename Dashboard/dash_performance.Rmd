...
<center>
```{r include = TRUE, echo=FALSE}
textMain <- paste0('Daily log return:' , round(tail(dm$logRetPort,1)*100,2),'% ',
       'Monthly log return:' , 
       round(sum(dm$logRetPort[paste0(Sys.Date()-30,'/',Sys.Date())])*100,2),'%')
close360d <- as.vector( dm$adjClosePort[Sys.Date()-360])

dygraphs::dygraph(dm$adjClosePort[paste0(Sys.Date()-360,'/',Sys.Date())]/ close360d, 
                  main =textMain) 

dygraphs::dygraph(tail(dp$vola$VIX, 360),  main = paste(unlist(strsplit(as.character(end(dp$vola$VIX))," ")),"VIX S&P 500 ",round(tail(dp$vola$VIX,1),4),"Pt. Change",round(diff(tail(dp$vola$VIX,2))[2],4))) %>%

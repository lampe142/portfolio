...
CAPM_return_D250 = sum(CAPM_return_D250))%>%
  ...
  formatStyle('return_D1', background = color_from_middle(t2$D1.LR,'pink','lightgreen'))%>%
  formatPercentage('CAPM_return_D1', 2)%>%
  formatStyle('CAPM_return_D1', background = color_from_middle(t2$D1.LR,'pink','lightgreen'))%>%
  formatCurrency('ab_return_D1', '\U20AC', digits = 0)  %>%
  
  formatPercentage('return_D5', 2)%>%
  formatStyle('return_D5', background = color_from_middle(t2$D1.LR,'pink','lightgreen'))%>%
  formatCurrency('ab_return_D5', '\U20AC', digits = 0)  %>%
  formatPercentage('CAPM_return_D5', 2)%>%
  formatStyle('CAPM_return_D5', background = color_from_middle(t2$D1.LR,'pink','lightgreen'))%>%
  formatCurrency('ab_return_D5', '\U20AC', digits = 0)  %>%
  
  formatPercentage('return_D23', 2)%>%
  formatStyle('return_D23', background = color_from_middle(t2$D1.LR,'pink','lightgreen'))%>%
  formatCurrency('ab_return_D23', '\U20AC', digits = 0) %>%
  formatPercentage('CAPM_return_D23', 2)%>%
  formatStyle('CAPM_return_D23', background = color_from_middle(t2$D1.LR,'pink','lightgreen'))%>%
  formatCurrency('ab_return_D23', '\U20AC', digits = 0)  %>%
  
  formatPercentage('return_D125', 2)%>%
  formatStyle('return_D125', background = color_from_middle(t2$D1.LR,'pink','lightgreen'))%>%
  formatCurrency('ab_return_D125', '\U20AC', digits = 0)   %>%
  formatPercentage('CAPM_return_D125', 2)%>%
  formatStyle('CAPM_return_D125', background = color_from_middle(t2$D1.LR,'pink','lightgreen'))%>%
  formatCurrency('ab_return_D125', '\U20AC', digits = 0)  %>%
  
  formatPercentage('return_D250', 2)%>%
  formatStyle('return_D250', background = color_from_middle(t2$D1.LR,'pink','lightgreen'))%>%
  formatCurrency('ab_return_D250', '\U20AC', digits = 0) %>%
  formatPercentage('CAPM_return_D250', 2)%>%
  formatStyle('CAPM_return_D250', background = color_from_middle(t2$D1.LR,'pink','lightgreen'))%>%
  formatCurrency('ab_return_D250', '\U20AC', digits = 0)
```
...
replace(dp$position, is.na(dp$position),0) %>% 
  subset((dp$position$SharePortfolio>0.001 &
           ...
           dp$position$Category == 'Benchmark' ,
                       select=c('Category','Name','CloseDate',
                                'SharePortfolio', 
                                'Value',
                                'beta','jensen_alpha',
                                'ab_return_D1',
                                ...
                                "return_D250",
                                "CAPM_return_D250"))%>% 
  plyr::arrange(Category,!SharePortfolio)  %>%
  datatable(extensions = 'Buttons', options = list(pageLength = 100,
  dom = 'Bfrtip', buttons = list('copy', 'csv', 'excel', 'pdf', 'print',
  list(extend = 'colvis',visible=F, targets= 0, columns = c(3:dim(t2)[2])))
  ))%>%
  formatCurrency('Value', '\U20AC', digits = 0) %>%
  formatCurrency('beta', '', digits = 3) %>%
  formatCurrency('jensen_alpha', '', digits = 2) %>%
  formatPercentage('SharePortfolio', 1) %>%
  formatStyle('SharePortfolio', background = color_from_middle(t2$SharePortfolio,'lightblue','lightblue')) %>%
...
```{r include = TRUE, echo=FALSE}
replace(dp$position, is.na(dp$position),0) %>% 
  subset((dp$position$SharePortfolio== 0 &
            !is.na(dp$position$SharePortfolio)) | dp$position$Name == 'Portfolio:'| 
           dp$position$Category == 'Benchmark' ,
                       select=c('Category','Name','CloseDate',
                                'SharePortfolio', 
